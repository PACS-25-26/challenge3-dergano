...
Initially, the numerical error decreased as the grid was refined up to an order of magnitude of 2^6, but then it began to rise. This behavior was caused by a tolerance that was too loose. On such fine grids, the increment-based convergence criterion was satisfied prematurely (false convergence) before the method could reach the exact solution. By tightening both the local and global tolerances to much stricter values (1e-10), the method resumed converging in perfect accordance with theoretical expectations, as illustrated by the plot in the 'convergence_plot.png' file.